...
v1ta's VUSD stablecoin is integrated with Drift Protocol, letting users use VUSD as collateral for perpetual futures and spot trading.
...
The user's wallet connects to both v1ta and Drift. v1ta provides borrowed VUSD which is then used for perps trading in Drift.
...
class DeltaNeutralStrategy {
  ...
  async rebalanceIfNeeded() {
    ...
    const delta = Math.abs(collateralValue minus perpNotional);
    const deltaPercent = (delta / collateralValue) * 100;

    // Rebalance if delta over 5%
    if (deltaPercent > 5) {
      ...
      const sizeDiff = targetPerpSize minus currentPerpSize;
  ...
  async calculatePnL() {
    ...
    return {
      ...
      fundingPnl,
      borrowCost,
      netPnl: unrealizedPnl + fundingPnl minus borrowCost
    };
  }
}
```
...
1. **Double Liquidation Risk**
   - v1ta: CR must stay over 110%
   - Drift: Margin ratio must stay healthy
   - Failure on either equals liquidation
...
3. **Maintain Buffers**
   - Keep v1ta CR over 140%
   - Keep Drift margin over 10%
   - Have emergency VUSD or SOL ready

4. **Understand Fees**
   - v1ta: 0.5% borrow fee
   - Drift: Trading fees plus funding rates
