...
**Purpose**: Modify existing CDP (add or remove collateral or debt).
...
2. After changes, CR at least 110%
3. Debt at least 1 VUSD (or 0 if closing)
...
**Purpose**: Close positions with CR under 110%.

**Eligibility**: Any position with CR under 110%

**Process**:
1. Verify CR under 110%
...
fn get_sol_price(price_feed: &AccountInfo) -> Result<u64> {
    ...
    // Check staleness
    require!(
        price_data.publish_time >= current_timestamp minus 60,
...
5. **Math operations**: No overflow or underflow
...
See the Security docs for full details and mitigation plans.

## Performance Considerations

### Transaction Costs

Approximate costs on Devnet:
- Initialize: about 0.01 SOL (one-time)
- Open Position: about 0.0005 SOL
- Adjust Position: about 0.0003 SOL
- Close Position: about 0.0003 SOL
- Liquidate: about 0.0005 SOL

### Compute Units

Typical compute unit usage:
- Open Position: about 150k CU
- Adjust Position: about 100k CU
- Liquidate: about 200k CU

All operations stay well within Solana's 1.4M CU limit per transaction.
