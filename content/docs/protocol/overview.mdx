...
v1ta is a decentralized borrowing protocol that allows you to mint VUSD stablecoin against SOL and LST collateral at just 110% collateralization ratio.
...
- **Maximum borrowing power**: Borrow up to ~90.9% of your collateral value
...
```mermaid
graph LR
    A[User] -->|Deposit SOL| B[CDP]
    B -->|Mint VUSD| C[User Wallet]
    B -->|Update State| D[Global State]
```
...
```mermaid
graph LR
    A[Liquidator] -->|Trigger| B[Unhealthy CDP]
    C[Stability Pool] -->|Repay Debt| B
    B -->|Transfer Collateral| C
    B -->|Reward 0.5%| A
```

1. Bot/user detects CR < 110%
...
```mermaid
graph LR
    A[User] -->|Burn VUSD| B[Protocol]
    B -->|Find Lowest CR CDP| C[CDP]
    C -->|Transfer SOL| A
    C -->|Update Debt| D[Global State]
```
